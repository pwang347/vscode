...
function createProvider(accessor: ServicesAccessor, type: AccessibleViewType): AccessibleContentProvider | undefined {
	...
	const help = [
		...
		recording
			? localize('computerUse.help.openRecording', "A recording opens when you activate its playback preview in chat. Every activation starts playback from the beginning.")
		...
		recording ? localize('computerUse.help.timeline', "The filled portion of the Recording Timeline marks elapsed footage. Use the slider to seek. Hover over the timeline or focus it with the keyboard to show a frame preview. Arrow keys adjust the focused slider, and playback resumes from the selected position. Dimmed sections mark periods where the captured image did not change. Action markers identify clicks, text entry, key presses, scrolling, dragging, secondary actions, and application launches. Tab to the action markers, use Left and Right Arrow to move between them, and press Enter or Space to seek.") : '',
		recording
			? localize('computerUse.help.recordingLastFrame', "When playback ends, the last recorded frame remains visible. Activate the recording's playback preview in chat to restart from the beginning.")
